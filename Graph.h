...
// Custom compare class for priotity queue
// (total_path_weight, from, to)
class Compare {
    public :bool operator()(const tuple<double, string, string>& first, 
                    const tuple<double, string, string>& second) const {
        return get<0>(first) > get<0>(second); // Compare based on path weight
    }
};
...
class Graph {
    public:
        ...
        vector<string> node_labels; // list of all nodes
        int total_edges; // total number of edges in graph 
        map<string, set<pair<string, double>>> adjacency_list; // representation of graph
        ...
        // Inserts a new edge into adjacency list
        void insert(string const & start_label, string const & end_label, double const & edge_weight);
